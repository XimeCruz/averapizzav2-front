...
import '../../providers/carrito_provider.dart'; // Importa TipoEntrega desde aquí
...
class _CarritoScreenState extends State<CarritoScreen> {
  ...
  Future<void> _confirmarPedido() async {
    ...
    try {
      final pedido = await carritoProvider.confirmarPedido(_metodoPagoSeleccionado!);

      if (!mounted) return;

      _mostrarExito(pedido);
    } catch (e) {
      if (!mounted) return;

      // Manejar error de stock insuficiente
      if (e.toString().contains('STOCK_INSUFICIENTE')) {
        _mostrarErrorStock(e.toString());
      } else {
  ...
  void _mostrarErrorStock(String errorJson) {
    try {
      final errorData = json.decode(errorJson.replaceFirst('Exception: ', ''));
      final insumo = errorData['insumo'];
      final disponible = errorData['disponible'];
      final requerido = errorData['requerido'];
      ...
      _mostrarError('Stock insuficiente para completar el pedido');
    }
  }

  void _mostrarExito(dynamic pedido) {
    final carritoProvider = context.read<CarritoProvider>();
        ...
        Container(
          ...
          child: SizedBox(
            ...
            child: ElevatedButton(
              onPressed: carrito.procesandoPedido ? null : _confirmarPedido,
              style: ElevatedButton.styleFrom(
              ...
              child: carrito.procesandoPedido
                  ? const SizedBox(
                height: 24,
                width: 24,
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: Column(
                children: [
                  _buildInfoRow('Total:', 'Bs. ${total.toStringAsFixed(2)}'),
                  const SizedBox(height: 12),
